...
router.delete("/:id", async function (req, res) {
	...
	return res.json({ success: true, message: "deleted portfolio" });
});
...
// Below code (router.patch) is not used in current build (2022.06.28), kept for possible future feature updates
...
router.patch("/:id", async function (req, res) {
	...
	if (keys[0] === "description") {
		result = await UpdateDescriptionPortfolio(
